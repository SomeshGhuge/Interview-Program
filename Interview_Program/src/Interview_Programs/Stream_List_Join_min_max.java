...
public class Stream_List_Join_min_max {
	...
	public static void main(String[]args) {
		...
		Stream<Integer>Joiner=Stream.concat(list1.stream(), list2.stream());
		List<Integer> All=Joiner.collect(Collectors.toList());
		System.out.println("All---->"+All);
		Set<Integer>Unique=All.stream().collect(Collectors.toSet());
		System.out.println("Unique--->"+Unique);
		System.out.println("Minimum Number----->"+Unique.stream().min((x1,x2)->x1-x2).get());
		System.out.println("Maximum Number----->"+Unique.stream().max((x1,x2)->x1-x2).get());
		System.out.println("Duplicate Numbers-->"+All.stream().filter(e->Collections.frequency(All,e )>1).collect(Collectors.toSet()));
